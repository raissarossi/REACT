...
const Input = ({ texto, tipo, obrigatorio, maxLength, act }) => {
  const [inputValue, setInputValue] = useState("");
  const [errorMsg, setErrorMsg] = useState("");


  function handleInputChange(event) {
    if(tipo == Number || tipo == 'num'){
  ...
  return (
    <>
      <input
        ...
        className={`form-control rounded-full m-4 p-[3px] ${inputWidth} items-center justify-center text-center`}
        placeholder={texto}
        maxLength={maxLength}
      />
